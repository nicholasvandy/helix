...
describe('Gene Reasoning (OPT-4)', () => {
  ...
  it('records failure analysis', () => {
    gm.recordFailureAnalysis('nonce-mismatch', 'nonce', 'RPC timeout during nonce refresh');
  ...
  it('keeps max 5 failure analysis entries', () => {
    for (let i = 0; i < 7; i++) gm.recordFailureAnalysis('nonce-mismatch', 'nonce', `fail-${i}`);
    const gene = gm.lookup('nonce-mismatch', 'nonce');
  ...
  it('updates success context', () => {
    gm.updateContext('nonce-mismatch', 'nonce', true, { chain: '8453', platform: 'coinbase' });
    const gene = gm.lookup('nonce-mismatch', 'nonce');
  ...
  it('updates failure context', () => {
    gm.updateContext('nonce-mismatch', 'nonce', false, { chain: '1' });
    const gene = gm.lookup('nonce-mismatch', 'nonce');
