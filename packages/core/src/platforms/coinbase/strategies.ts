...
export function coinbaseConstruct(failure: FailureClassification): RepairCandidate[] {
  ...
  if (failure.platform === 'coinbase' && (failure.category as string) === 'gas') {
    return [
      ...
      { id: 'cb_fix_gas', strategy: 'fix_params', description: 'Auto-populate gas parameters', estimatedCostUsd: 0.01, estimatedSpeedMs: 100, requirements: [], score: 0, successProbability: 0.85, platform: 'coinbase' },
    ];
  }
